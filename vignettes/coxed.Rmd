...
We can calculate standard errors and confidence intervals for any of these quantities with the `bootstrap=TRUE` option.  
```{r npsfbs, cache=TRUE}
...
summary(ed1, stat="median")
```
By default the bootstrapping procedure uses 200 iterations (to set this value to a different number, use the `B` argument).
```{r npsfbs2, cache=TRUE}
ed1 <- coxed(mv.cox, method="npsf", bootstrap = TRUE, B=500)
summary(ed1, stat="mean")
summary(ed1, stat="median")
```
To change the confidence interval to a different level, use the `level` argument:
```{r npsfbs3, cache=TRUE}
ed1 <- coxed(mv.cox, method="npsf", bootstrap = TRUE, level=.8)
summary(ed1, stat="mean")
summary(ed1, stat="median")
```
...
```{r npsfbs4, cache=TRUE}
ed1 <- coxed(mv.cox, method="npsf", bootstrap = TRUE, B=1000, confidence="empirical")
summary(ed1, stat="mean")
summary(ed1, stat="median")
```
...
```{r coxedoutofsample, cache=TRUE}
forecast <- coxed(mv.cox, newdata=new.coalitions, method="npsf", bootstrap=TRUE)
forecast$exp.dur
```
...
We can use the GAM method to for all of the same uses for which we used the NPSF method above, except for estimating the baseline functions. We can however view and plot the output from the GAM model that maps predicted ranks to duration.
...
## With (bootstrapped) standard errors
Bootstrapping works with the GAM method in the same way that it works with the NPSF method. Setting `bootstrap=TRUE` provides a standard error and confidence interval for each predicted duration,
```{r gambs, cache=TRUE}
ed2 <- coxed(mv.cox, method="gam", bootstrap = TRUE)
head(ed2$exp.dur)
```
and for the mean and median duration.
```{r gambssummary}
summary(ed2, stat="mean")
summary(ed2, stat="median")
```
...
```{r coxedoutofsample2, cache=TRUE}
forecast <- coxed(mv.cox, newdata=new.coalitions, method="gam", bootstrap=TRUE)
forecast$exp.dur
```

## Marginal changes in expected duration
Here we again calculate the two marginal effects to better understand the substantive meaning of the Cox model.  This time we employ the GAM method instead of the NPSF method. Note that the GAM method provides a warning that some observations have linear predictors that are greater than or less than all of the observed cases in the estimation sample. Some observations falling outside the range of the original linear predictors is to be expected when applying new data, but if it happens with too many of the new observations NPSF may be a better option for estimating these quantities.
```{r gammebs, cache=TRUE}
...
```{r tvcme, cache=TRUE}
me <- coxed(bs.cox, method="npsf", bootstrap = TRUE,
